...
public static class PromptTemplateLibrary
{
    ...
    public static IReadOnlyList<PromptTemplateDefinition> Builtin { get; } = new[]
    {
        ...
        new PromptTemplateDefinition
        {
            ...
            OutputMode = "replace",
            BuiltIn = true,
            Description = "只调整空白与换行。本地已有同名智能动作，此模板走 AI",
            // 与本地 TextTidy 四条硬规则对齐，避免模型和内置动作各搞一套
            Prompt = "请整理下面文本的格式，只调整空白与换行，不得修改、增删、翻译任何文字。要求：\n"
                   + "1. 一行内连续空格（含 tab）压成 1 个空格；连续全角空格压成 1 个全角空格。\n"
                   + "2. 连续两个及以上换行压成 1 个换行。\n"
                   + "3. 去掉行末空白。\n"
                   + "4. 行末是逗号（, ，、）时去掉随后换行，与下一行拼接；英文逗号后若下行以字母开头则补一个空格，中文不加空格。\n"
                   + "5. ``` / ~~~ 代码围栏内部原样不动。\n"
