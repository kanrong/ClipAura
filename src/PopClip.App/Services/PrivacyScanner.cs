...
/// RuleId 是规则编号，便于"按规则忽略"等高级路径后续叠加</summary>
...
/// - 规则集 7 条内置（手机号 / 身份证 / 邮箱 / 银行卡 + Luhn / API Key / JWT / Windows 路径用户名），
///   全部作为程序默认启用，不再向 UI 暴露"逐条开关 / 自定义规则"，避免设置面板被次要选项淹没；
///   误打码扰民由规则本身严谨度（如 Luhn 校验、前后非数字断言）控制。
...
internal sealed class PrivacyScanner
{
    ...
    /// <summary>把扫描时需要的 settings 字段冷冻成一份只读快照，避免扫描运行期间被并发改动。
    /// 当前仅依赖总开关，规则集走内置默认</summary>
    public readonly record struct AppSettingsSnapshot(bool Enabled);
    ...
    public IReadOnlyList<PrivacyHit> Scan(OcrResult result)
    {
        ...
        if (_builtin.Count == 0) return Array.Empty<PrivacyHit>();
