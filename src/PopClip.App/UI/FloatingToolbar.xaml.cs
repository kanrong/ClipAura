...
public partial class FloatingToolbar : Window, INotifyPropertyChanged, INotificationSink
{
    ...
    /// <summary>窗口尺寸 = ContentHost.DesiredSize + 这个值。
    ...
    public void ApplyAppearance(AppSettings settings)
    {
        ...
        Dispatcher.Invoke(() =>
        {
            ...
            // 外壳圆角 = 用户设置 radius + 1，给 1px 边框留出抗锯齿余量；阴影底板与内容层共用此外圆角
            Resources["ToolbarCornerRadius"] = new CornerRadius(ResolveOuterCornerRadius(radius));
            // hover 高亮块的圆角直接取用户设置值：设置接近方形时块也接近方形，
            // 块四周由 ToolbarContentPadding 与按钮 Margin 留白，无需任何裁切也不会缺角
            Resources["ToolbarButtonCornerRadius"] = new CornerRadius(radius);
            // spacing 仅作用于按钮的左右 Margin，决定相邻 hover 块的水平间距；
            // 按钮 Padding 决定块内图标/文字的内边距（随密度档变化）
    ...
    private (double WidthDip, double HeightDip) GetPreferredWindowSizeDip()
    {
        ...
        var contentWidth = ContentHost.DesiredSize.Width > 0
            ? ContentHost.DesiredSize.Width
            : ContentHost.ActualWidth;
        var contentHeight = ContentHost.DesiredSize.Height > 0
            ? ContentHost.DesiredSize.Height
