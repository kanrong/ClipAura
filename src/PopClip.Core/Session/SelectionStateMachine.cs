        ...
        switch (ev)
        {
            ...
            case MouseUpEvent mu:
                ...
                else if (_movedFarEnough)
                {
                    ...
                    // 仅此分支传递 IsLikelyWindowDrag / IsLikelyScrollBarDrag：只有"鼠标移动超过阈值"
                    // 的拖动才可能是拖窗体或拖滚动条，其他分支（修饰键点击、Shift 原地点击、双击）
                    // 鼠标几乎没动，不会命中这两类非文本拖动
                    TryScheduleSelection(new SelectionCandidate(SelectionTrigger.MouseDrag, mu.X, mu.Y, mu.TimestampUtc, mu.IsLikelyWindowDrag, mu.IsLikelyScrollBarDrag), mu);
                    _lastUpAtUtc = DateTime.MinValue;
                }
